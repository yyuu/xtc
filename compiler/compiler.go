...
func (self *Compiler) evaluateAST(ast *bs_ast.AST) {
  ...
  sem := self.semanticAnalyze(ast, types)
  self.dumpSemant(sem)
  ir := NewIRGenerator(self.errorHandler, self.options, types).Generate(sem)
  self.dumpIR(ir)
  asm := self.generateAssembly(ir)
  self.dumpAsm(asm)
  self.printAsm(asm)
}

func (self *Compiler) semanticAnalyze(ast *bs_ast.AST, types *bs_typesys.TypeTable) *bs_ast.AST {
...
func (self *Compiler) generateAssembly(ir *bs_ir.IR) bs_sysdep.AssemblyCode {
